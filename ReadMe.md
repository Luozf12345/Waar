...
无需安装开发环境，也**不需要会使用 Git**。

### 推荐：从 GitHub Releases 下载

macOS 的 `.app` 在 GitHub 仓库里会显示为**文件夹**，浏览器无法直接下载。请前往：

**[GitHub Releases 下载页](https://github.com/Luozf12345/Waar/releases)**

在最新版本（Latest）中，按你的 Mac 芯片选择对应文件：

| 你的 Mac | 下载文件 |
|----------|----------|
| M 系列（Apple Silicon） | `waar_mac_apple_release.zip` |
| Intel 芯片 | `waar_mac_intel_release.zip` |
| Android 手机 | `waar_android_release.apk` |

下载后：

1. 解压 `.zip`（双击即可）
2. 将解压出的 `waar_mac_*_release.app` 拖到「应用程序」文件夹，或任意位置
3. 首次打开若提示「无法验证开发者」，请前往 **系统设置 → 隐私与安全性**，点击 **仍要打开**；或在 Finder 中右键该应用，选择 **打开**

> 如果 Releases 页面暂时没有文件，说明新版本还在准备中，可先扫码加入反馈群获取，或联系 **zflyluo**。

### Android

1. 在 [Releases](https://github.com/Luozf12345/Waar/releases) 下载 `waar_android_release.apk`，或从仓库 `app/` 目录获取
2. 将 APK 传输到手机
3. 在系统设置中允许「安装未知来源应用」（各厂商名称略有不同）
4. 点击 APK 完成安装并打开

### macOS 补充说明

- **不要**在 GitHub 仓库页面里点进 `app/waar_mac_*.app` 文件夹尝试下载——那是给开发者看的源码目录结构，不适合普通用户
- 普通用户请始终从 **Releases** 下载 `.zip` 单文件
...
| macOS Apple Silicon | `app/waar_mac_apple_release.app` + `.zip` |
| macOS Intel | `app/waar_mac_intel_release.app` + `.zip` |
| Windows | `app/waar_windows_release/` |

macOS 构建时会自动生成 `.zip`，也可对已有产物执行：

```bash
./scripts/package_release.sh
```

### 6. 发布到 GitHub Releases（维护者）

普通用户应从 Releases 下载，不要把 `.app` 文件夹直接发给用户。发布流程：

```bash
# 1. 在 Apple Silicon / Intel Mac 上分别构建 release
cd code/waar_window_flutter
./build.sh release

# 2. 若需补打 zip（构建脚本已自动生成，此步通常可跳过）
cd ../..
./scripts/package_release.sh

# 3. 创建 Release 并上传（将 v0.0.1 换成实际版本号）
gh release create v0.0.1 \
  app/waar_android_release.apk \
  app/waar_mac_apple_release.zip \
  app/waar_mac_intel_release.zip \
  --title "v0.0.1" \
  --notes "见 ChangeLog.md"
```

也可在 GitHub 网页 **Releases → Draft a new release** 中手动上传上述文件。
...
│   ├── waar_mac_apple_release.zip   # 供 Releases 分发
